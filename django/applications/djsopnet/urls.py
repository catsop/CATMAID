...
urlpatterns += patterns('djsopnet.control.segment',
    ...
    (r'^(?P<project_id>\d+)/segmentation/(?P<segmentation_stack_id>\d+)/segments/constraints$', 'retrieve_constraints'),
...
urlpatterns += patterns('djsopnet.control.slice',
    ...
    (r'^(?P<project_id>\d+)/segmentation/(?P<segmentation_stack_id>\d+)/slices/mask/(?P<slice_hash>\d+).png$',
        'slice_mask'),
