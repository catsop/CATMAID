...
urlpatterns = patterns('djsopnet.views',
    ...
    (r'^(?P<project_id>\d+)/stack/(?P<stack_id>\d+)/retrieve_conflict_sets$', 'retrieve_conflict_sets'),
    (r'^(?P<project_id>\d+)/stack/(?P<stack_id>\d+)/retrieve_associated_block_ids$', 'get_blocks_by_slice'),
