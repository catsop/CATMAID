...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.execute('''
                ALTER TABLE segstack_template.segment_slice
                  DROP CONSTRAINT segment_slice_pkey,
                  ADD PRIMARY KEY (segment_id, slice_id);
                ''')

        for segstack_id in SegmentationStack.objects.all().values_list('id', flat=True):
            db.execute('''
                    ALTER TABLE segstack_%s.segment_slice
                      DROP CONSTRAINT segment_slice_pkey,
                      ADD PRIMARY KEY (segment_id, slice_id);
                    ''' % segstack_id)
    ...
    def backwards(self, orm):
        db.execute('''
                ALTER TABLE segstack_template.segment_slice
                  DROP CONSTRAINT segment_slice_pkey,
                  ADD PRIMARY KEY (slice_id, segment_id);
                ''')
