...
class Migration(SchemaMigration):
    def forwards(self, orm):
        db.execute('''
            DROP RULE IF EXISTS djsopnet_segment_on_duplicate_ignore
            ON djsopnet_segment
        ''')
        db.execute('''
            DROP RULE IF EXISTS djsopnet_slice_on_duplicate_ignore
            ON djsopnet_slice
        ''')
        db.execute('''
            DROP RULE IF EXISTS djsopnet_segmentslice_on_duplicate_ignore
            ON djsopnet_segmentslice
        ''')
        db.execute('''
            DROP RULE IF EXISTS djsopnet_segmentfeatures_on_duplicate_ignore
            ON djsopnet_segmentfeatures
        ''')
        db.execute('''
    ...
    def backwards(self, orm):
        # Ignore inserts of segments with same ID
        db.execute('''
            CREATE OR REPLACE RULE djsopnet_segment_on_duplicate_ignore
            AS ON INSERT TO djsopnet_segment
            WHERE EXISTS (SELECT 1 FROM djsopnet_segment WHERE id=NEW.id)
            DO INSTEAD NOTHING
        ''')
        # Ignore inserts of slices with same ID
        db.execute('''
            CREATE OR REPLACE RULE djsopnet_slice_on_duplicate_ignore
            AS ON INSERT TO djsopnet_slice
            WHERE EXISTS (SELECT 1 FROM djsopnet_slice WHERE id=NEW.id)
            DO INSTEAD NOTHING
        ''')

        # Ignore inserts of segments-slice associations
        db.execute('''
            CREATE OR REPLACE RULE djsopnet_segmentslice_on_duplicate_ignore
            AS ON INSERT TO djsopnet_segmentslice
            WHERE EXISTS (SELECT 1 FROM djsopnet_segmentslice WHERE segment_id=NEW.segment_id AND slice_id=NEW.slice_id)
            DO INSTEAD NOTHING
        ''')
        db.execute('''
            CREATE OR REPLACE RULE djsopnet_segmentfeatures_on_duplicate_ignore
            AS ON INSERT TO djsopnet_segmentfeatures
            WHERE EXISTS (SELECT 1 FROM djsopnet_segmentfeatures WHERE segment_id=NEW.segment_id)
            DO INSTEAD NOTHING
        ''')
