...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Changing field 'AssemblyEquivalence.skeleton'
        db.alter_column(u'djsopnet_assemblyequivalence', 'skeleton_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['catmaid.ClassInstance'], null=True))

    def backwards(self, orm):
        raise RuntimeError("Cannot reverse this migration.")
        # Run the below code in two transactions and remove the above error if
        # you need to reverse this migration. The NULL values cannot be removed
        # and the column altered within one migration.

        # WARNING: deletes equivalences with NULL skeletons
        db.execute('''
            UPDATE djsopnet_assembly SET equivalence_id = NULL
            WHERE EXISTS (
                SELECT 1 FROM djsopnet_assemblyequivalence ae
                WHERE ae.id = djsopnet_assembly.equivalence_id
                    AND ae.skeleton_id IS NULL);
            DELETE FROM djsopnet_assemblyequivalence
            WHERE skeleton_id IS NULL;
            ''')

        # Changing field 'AssemblyEquivalence.skeleton'
        db.alter_column(u'djsopnet_assemblyequivalence', 'skeleton_id', self.gf('django.db.models.fields.related.ForeignKey')(default=0, to=orm['catmaid.ClassInstance']))
