...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        for segstack_id in SegmentationStack.objects.all().values_list('id', flat=True):
            db.execute('''
                    ALTER TABLE segstack_%(segstack_id)s.solution_precedence
                    ADD CONSTRAINT solution_precedence_core_id_fkey
                      FOREIGN KEY (core_id) REFERENCES segstack_%(segstack_id)s.core(id),
                    ADD CONSTRAINT solution_precedence_solution_id_fkey
                      FOREIGN KEY (solution_id) REFERENCES segstack_%(segstack_id)s.solution(id)
    ...
    def backwards(self, orm):
        for segstack_id in SegmentationStack.objects.all().values_list('id', flat=True):
            db.execute('''
                    ALTER TABLE segstack_%(segstack_id)s.solution_precedence
                    DROP CONSTRAINT IF EXISTS solution_precedence_core_id_fkey,
                    DROP CONSTRAINT IF EXISTS solution_precedence_solution_id_fkey
                    ''' % {'segstack_id': segstack_id})
