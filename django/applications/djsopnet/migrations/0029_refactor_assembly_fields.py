...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Assembly.creation_time'
        db.delete_column(u'djsopnet_assembly', 'creation_time')

        # Deleting field 'Assembly.edition_time'
        db.delete_column(u'djsopnet_assembly', 'edition_time')

        # Deleting field 'Assembly.user'
        db.delete_column(u'djsopnet_assembly', 'user_id')

        # Adding field 'Assembly.solution'
        db.add_column(u'djsopnet_assembly', 'solution',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=None, null=True, to=orm['djsopnet.Solution']),
                      keep_default=False)
        db.execute('''
            UPDATE djsopnet_assembly a SET solution_id = (
                SELECT ssol.solution_id FROM djsopnet_segmentsolution ssol
                WHERE ssol.assembly_id = a.id LIMIT 1)
            ''')
        db.execute('''ALTER TABLE djsopnet_assembly ALTER COLUMN solution_id SET NOT NULL''')


    def backwards(self, orm):
        # Get a junk but valid user ID
        default_user_id = User.objects.all()[0].id

        # Adding field 'Assembly.creation_time'
        db.add_column(u'djsopnet_assembly', 'creation_time',
                      self.gf('django.db.models.fields.DateTimeField')(default=datetime.datetime.now),
                      keep_default=False)

        # Adding field 'Assembly.edition_time'
        db.add_column(u'djsopnet_assembly', 'edition_time',
                      self.gf('django.db.models.fields.DateTimeField')(default=datetime.datetime.now),
                      keep_default=False)

        # Adding field 'Assembly.user'
        db.add_column(u'djsopnet_assembly', 'user',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=default_user_id, to=orm['auth.User']),
                      keep_default=False)

        # Deleting field 'Assembly.solution'
        db.delete_column(u'djsopnet_assembly', 'solution_id')
