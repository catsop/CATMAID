...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.execute('''
                ALTER TABLE segstack_template.segment
                DROP COLUMN ctr_x,
                DROP COLUMN ctr_y
                ''')
        for segstack_id in SegmentationStack.objects.all().values_list('id', flat=True):
            db.execute('''
                    ALTER TABLE segstack_%(segstack_id)s.segment
                    DROP COLUMN ctr_x,
                    DROP COLUMN ctr_y
    ...
    def backwards(self, orm):
        db.execute('''
                ALTER TABLE segstack_template.segment
                ADD COLUMN ctr_x double precision NOT NULL,
                ADD COLUMN ctr_y double precision NOT NULL
                ''')
        for segstack_id in SegmentationStack.objects.all().values_list('id', flat=True):
            db.execute('''
                    ALTER TABLE segstack_%(segstack_id)s.segment
