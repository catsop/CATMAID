...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        db.execute('''
                CREATE INDEX assembly_equivalence_id_idx
                  ON segstack_template.assembly
                  USING btree (equivalence_id);
                ''')

        for segstack_id in SegmentationStack.objects.all().values_list('id', flat=True):
            db.execute('''
                CREATE INDEX assembly_equivalence_id_idx
                  ON segstack_%s.assembly
                  USING btree (equivalence_id);
                    ''' % segstack_id)

    def backwards(self, orm):
        db.execute('''
                DROP INDEX segstack_template.assembly_equivalence_id_idx;
                ''')

        for segstack_id in SegmentationStack.objects.all().values_list('id', flat=True):
            db.execute('''
                DROP INDEX segstack_%s.assembly_equivalence_id_idx;
                    ''' % segstack_id)
