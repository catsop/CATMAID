...
from djsopnet.fields import ConstraintRelationEnumField


class Migration(SchemaMigration):

    def forwards(self, orm):
        db.execute('CREATE TYPE constraintrelation AS ENUM (' + \
             ','.join([("'%s'" % v[0]) for v in ConstraintRelationEnumField.enum_choices]) + ')')

        # Adding field 'Constraint.relation'
        db.add_column(u'djsopnet_constraint', 'relation',
                      self.gf('djsopnet.fields.ConstraintRelationEnumField')(default='Equal'),
                      keep_default=False)

        # Adding field 'Constraint.value'
        db.add_column(u'djsopnet_constraint', 'value',
                      self.gf('django.db.models.fields.FloatField')(default=1.0),
                      keep_default=False)

        # Adding field 'ConstraintSegmentRelation.coefficient'
        db.add_column(u'djsopnet_constraintsegmentrelation', 'coefficient',
                      self.gf('django.db.models.fields.FloatField')(default=1.0),
                      keep_default=False)


    def backwards(self, orm):
        # Deleting field 'Constraint.relation'
        db.delete_column(u'djsopnet_constraint', 'relation')

        # Deleting field 'Constraint.value'
        db.delete_column(u'djsopnet_constraint', 'value')

        # Deleting field 'ConstraintSegmentRelation.coefficient'
        db.delete_column(u'djsopnet_constraintsegmentrelation', 'coefficient')

        db.execute('DROP TYPE constraintrelation')
